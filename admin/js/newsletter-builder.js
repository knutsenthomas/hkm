...
class NewsletterBuilder {
    ...
    async loadDraftFromUrl(draftId) {
        ...
        try {
            ...
            if (doc.exists) {
                ...
                this.currentDraftName = data.name || 'Utkast';
    ...
    renderCanvas() {
        ...
        if (this.blocks.length === 0) {
            ...
            if (autosavedHtml && autosavedHtml.includes("Sommerbasar") && autosavedHtml.includes("His Kingdom Designs")) {
        ...
        if (isUnified) {
            ...
            if (!rawText.includes("Andakt") || !rawText.includes("Lyset i hverdagen") || !rawText.includes("SOMMER-BASAREN")) {
                ...
                this.blocks = [{ id: 'unified_content', type: 'text', content: { text: fullRichHtml } }];
            }
            unifiedHtml = rawText;
        } else {
    ...
    compileSocialBlockForEmail(block, lang = 'no') {
        ...
        if (selectedStyle === 'text_only') {
            ...
            const platformIconMap = {
                facebook: 'https://img.icons8.com/ios-filled/96/1B4965/facebook-new.png',
                instagram: 'https://img.icons8.com/ios-filled/96/1B4965/instagram-new.png',
                youtube: 'https://img.icons8.com/ios-filled/96/1B4965/youtube-play.png',
                website: 'https://img.icons8.com/ios-filled/96/1B4965/domain.png'
            };

            linksMarkup = platforms.map(platform => {
                const iconSrc = platformIconMap[platform.id] || platform.image || 'https://img.icons8.com/ios-filled/96/1B4965/domain.png';
                const borderRadius = selectedStyle === 'icon_only' ? '50%' : '16px';
                return `
                    <td align="center" valign="middle" style="padding: 4px 7px;">
                        <a href="${platform.href}" target="_blank" title="${platform.label}" aria-label="${platform.label}" style="display: inline-block; width: 54px; height: 54px; border: 1px solid #e2e8f0; border-radius: ${borderRadius}; background-color: #ffffff; text-align: center; text-decoration: none; box-shadow: 0 4px 10px rgba(0,0,0,0.06);">
                            <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="54" height="54" style="width: 54px; height: 54px; border-collapse: collapse;">
                                <tr>
                                    <td align="center" valign="middle" style="text-align: center; vertical-align: middle; width: 54px; height: 54px; padding: 0;">
                                        <img src="${iconSrc}" width="26" height="26" alt="${platform.label}" style="display: block; margin: 0 auto; width: 26px; height: 26px; border: 0; outline: none; text-decoration: none;">
                                    </td>
                                </tr>
                            </table>
                        </a>
    ...
    compileEmailHtml(lang = 'no') {
        ...
        if (isEnglish) {
            ...
            // The visible canvas contains English while editing that language.
            // Build the Norwegian email from its preserved canonical blocks.
            const tempDiv = document.createElement('div');
            tempDiv.innerHTML = this.getLanguageBlocksHtml(this.blocks || []);
            contentClone = tempDiv;
        } else {
...
Svar KUN med et gyldig JSON-objekt (ingen markdown kodelister som \`\`\`json, svar kun med ren tekst/JSON) i dette formatet:
{
  ...
  "html": "Oversatt engelsk HTML-innhold"
    ...
    async sendCampaign() {
        ...
        const visibleSubject = document.getElementById('newsletter-subject')?.value?.trim() || '';
        this.syncUnifiedBlocks();
        const subject = this.currentEditorLang === 'en'
            ? (this.subjectNo || '').trim()
            : visibleSubject;
        const norwegianSource = this.getLanguageBlocksHtml(this.blocks || []);
        const plainText = norwegianSource.replace(/<[^>]*>/g, '').replace(/&nbsp;/g, '').trim();

        if (!subject || subject === '') {
            if (typeof showToast === 'function') {
                showToast("Vennligst skriv inn et norsk emne for nyhetsbrevet.", "warning");
            }
            return;
        }

        if (this.blocks.length === 0 || plainText === '') {
        ...
        try {
            ...
            if (finalBtn) {
                finalBtn.disabled = true;
                finalBtn.innerHTML = `<span class="material-symbols-outlined rotating">sync</span> Legger ${recipients.length} mottakere i utsendingskø...`;
            }

            // Compile both language variants before handing the campaign to the
            // server. The server continues even if the browser is closed.
            ...
            if (!window.firebaseService?.db) {
                throw new Error('Databasen for utsendingskøen er ikke tilgjengelig.');
            }
            ...
            const campaignData = {
                ...
                recipients: recipients
                    ...
                    .map(recipient => ({
                        ...
                        tags: Array.isArray(recipient.tags) ? recipient.tags : [],
                        segments: Array.isArray(recipient.segments) ? recipient.segments : []
                    })),
                status: 'queued',
            ...
            await this.recordNewsletterVersion('Kampanje lagt i utsendingskø', {
                minIntervalMs: 0,
            ...
            if (typeof showToast === 'function') {
                showToast(`Kampanjen er lagt i kø for ${recipients.length} mottakere. Serveren sender den nå.`, "success");
            }
            if (finalBtn) {
                finalBtn.disabled = false;
                finalBtn.innerHTML = originalText;
            ...
            const confirmedBack = await this.showConfirm(
                'Utsending startet!',
                `Nyhetsbrevet er lagt trygt i utsendingskøen for ${recipients.length} abonnenter. Utsendingen fortsetter på serveren selv om du lukker editoren.\n\nVil du gå tilbake til oversikten?`,
                'Gå til oversikt',
                'Bli i editoren'
            );
            if (confirmedBack) {
    ...
    async loadSubscribers() {
        ...
        try {
            ...
            const subscribersList = await this.fetchSubscribersList();
                ...
                if (campaignSnap) {
                    campaignSnap.forEach(doc => {
                        ...
                        rawCampaigns.push({
                            ...
                            type: 'newsletter',
                            author: data.sentBy || data.senderName || 'His Kingdom Ministry',
                ...
                <div class="card-body">
                    ...
                    <div class="card-footer" style="display: flex !important; align-items: center !important; justify-content: space-between !important; padding-top: 14px !important; border-top: 1px solid #f1f5f9 !important; gap: 12px !important;">
                        <div style="display: flex !important; gap: 12px !important; align-items: center !important; min-width: 0 !important; flex: 1 !important;">
                            <div class="card-meta-item" style="white-space: nowrap !important; flex-shrink: 0 !important; display: inline-flex !important; align-items: center !important;">
                                <span class="material-symbols-outlined">calendar_today</span>
                                <span>${dateStr}</span>
                            </div>
                            <div class="card-meta-item" style="white-space: nowrap !important; min-width: 0 !important; max-width: 140px !important; overflow: hidden !important; text-overflow: ellipsis !important; display: inline-flex !important; align-items: center !important;">
