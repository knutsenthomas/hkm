...
function normalizeFacebookPost(post, index, fallbackPageUrl) {
  ...
  const mainText = cleanMessage || cleanTitle || cleanDesc || cleanStory || "";
  const lines = mainText.split(/\n+/).map((line) => line.trim()).filter(Boolean);
  const rawTitle = lines[0] || cleanTitle || cleanStory || fallbackTitle;
  const title = trimText(rawTitle, 78);
  const excerptSource = lines.length > 1 ? lines.slice(1).join(" ") : (cleanDesc || cleanStory || (mainText !== rawTitle ? mainText : ""));
    ...
    for (const userDoc of usersSnap.docs) {
      ...
      for (const activePlanDoc of activePlansSnap.docs) {
        ...
        const readingContentHtml = `
          <div style="background-color: #FCF9F5;">
            <!-- Personal Greeting -->
            <section style="margin-bottom: 32px; text-align: center; max-width: 440px; margin-left: auto; margin-right: auto;">
  ...
  <footer style="padding: 24px 20px 32px 20px; box-sizing: border-box; max-width: 100%; background-color: rgba(252, 249, 245, 0.5); text-align: center; border-top: 1px solid rgba(221, 193, 182, 0.1);">
    ...
    <a href="https://www.hiskingdomministry.no/minside" style="font-family: 'Inter', sans-serif; font-size: 9px; text-transform: uppercase; letter-spacing: 0.15em; color: #1B4965; text-decoration: underline; text-underline-offset: 4px; font-weight: 600;">
      Endre dine varslingsinnstillinger
          ...
          if (template && template.body && !template.body.includes('#FCF9F5')) {
          ...
          if (html.includes("{{reading_content}}")) {
            ...
            html = `
              <div class="hkm-email-container" style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; width: 100%; max-width: 600px; box-sizing: border-box; margin: 0 auto; background-color: #FCF9F5; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid #EAE4DC; text-align: left;">
                ...
                <!-- Editorial Header -->
                <header style="padding: 64px 48px 32px 48px; text-align: center; background-color: #FCF9F5; border-bottom: none;">
                  <div style="margin-bottom: 24px;">
                ...
                <!-- Body -->
                <div class="hkm-email-body" style="padding: 0 48px 64px 48px; background-color: #FCF9F5; color: #121c2c;">
                  \${html}
                </div>
...
exports.onCourseEnrollmentUpdatedTrigger = onDocumentUpdated({
  ...
  // Sjekk om status endret seg til 'paid' eller 'success' (godkjent tilgang)
  const isNowApproved = (afterData.status === 'paid' || afterData.status === 'success') && 
                        (beforeData.status !== 'paid' && beforeData.status !== 'success');

  if (isNowApproved && afterData.email) {
...
exports.notifyCourseParticipants = onRequest({ cors: true, secrets: [emailUserParam, emailPassParam] }, async (req, res) => {
  ...
  await verifyAdmin(req, res, async () => {
    try {
      ...
      const fallbackRecording = {
        subject: subject || `🎉 Nytt opptak er lagt ut: ${title}`,
        body: `<div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; width: 100%; max-width: 600px; box-sizing: border-box; margin: 0 auto; background-color: #FCF9F5; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); border: 1px solid #EAE4DC; text-align: left;">
  <div style="height: 4px; background: linear-gradient(90deg, #1B4965 0%, #d17d39 50%, #bd4f2a 100%);"></div>
  ...
  <div style="padding: 0 24px 36px 24px; box-sizing: border-box; background-color: #FCF9F5;">
    <section style="margin-bottom: 24px; text-align: center;">
      ...
      <h2 style="font-family: 'Merriweather', Georgia, serif; font-size: 24px; line-height: 32px; font-weight: 700; color: #121c2c; margin: 0 0 8px 0;">{{courseTitle}}</h2>
    </section>
    ...
    <section style="border-top: 1px solid rgba(221, 193, 182, 0.2); padding-top: 24px; font-size: 13px; color: #64748b;">
      ...
      const templateRecording = await getEmailTemplate("course_recording_notification", fallbackRecording);

      const emailPromises = recipients.map(async (r) => {
        const customMsgFormatted = message.replace(/{{name}}/g, r.name || 'kursdeltaker')
                                           .replace(/{{courseTitle}}/g, title);

        const emailSubject = (subject || templateRecording.subject)
          .replace(/{{name}}/g, r.name || 'kursdeltaker')
          .replace(/{{courseTitle}}/g, title);

        const html = templateRecording.body
          .replace(/{{name}}/g, r.name || 'kursdeltaker')
          .replace(/{{courseTitle}}/g, title)
          .replace(/{{message}}/g, customMsgFormatted.replace(/\n/g, '<br>'))
          .replace(/{{actionUrl}}/g, link);

        try {
          await sendEmail({ to: r.email, subject: emailSubject, html, text: customMsgFormatted });
