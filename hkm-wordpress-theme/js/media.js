...
function initMediaNavigation() {
    ...
    const isMediaPage = currentPath.includes('media.html') || currentPath.includes('/media/') || currentPath.endsWith('/media');
    const tabs = document.querySelectorAll('.media-tab');

    tabs.forEach(tab => {
        const href = tab.getAttribute('href');
        if (href === currentPath || (href.includes('media') && isMediaPage)) {
    ...
    if (mediaCards.length > 0 && isMediaPage) {
...
function renderPodcastEpisodes() {
    ...
    const isFullPage = window.location.pathname.includes('podcast.html') || window.location.pathname.includes('/podcast/') || window.location.pathname.endsWith('/podcast');
